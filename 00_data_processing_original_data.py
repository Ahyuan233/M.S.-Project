...
for filename in os.listdir(folder_path):
    ...
    if os.path.isfile(file_path):
        ...
        # vis = o3d.visualization.Visualizer()
        # vis.create_window()
        # vis.add_geometry(pcd_left) #pcd_right
        # vis.run()
        # vis.destroy_window()
        ...
        pcd_left_crop_for_ICP = pcd_left.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([-90, -72, -105]), # min  x-70 is the left dide of the slope, 55 is the right side of the slope ,y:-72 top of the ball, 70 bottom of the slope, z:from -150 near to 500 far, -105 is the position without camera plane, 210 is the start of slope
                                                                          np.array([90, 110, 210])))  # max
        
        pcd_right_crop_for_ICP = pcd_right.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([-90, -72, -105]), # min  x-70 is the left dide of the slope, 55 is the right side of the slope ,y:-72 top of the ball, 70 bottom of the slope, z:from -150 near to 500 far, -105 is the position without camera plane, 210 is the start of slope
                                                                          np.array([90, 110, 210])))  # max
        ...
        # vis = o3d.visualization.Visualizer()
        # vis.create_window()
        # vis.add_geometry(pcd_right_crop)
        # vis.run()
        # vis.destroy_window()
        ...
        # draw_registration_result_original_color(source_down, target_down, opt_transformation)
