...
#       can obtain colored point cloud by combining the color and pcd; need to capture & save only the moving robot. One idea is to apply a green cloth on the scene and ingnore all the green points during data collection
...
class KINECT():
    ...
    def distance_filter(self, point_cloud, distance_threshold):
        ...
        # extract only points of white color

    def color_filter(self, point_cloud_points, point_cloud_colors, low_color, high_color):
        # Load the point cloud
        # pcd = o3d.io.read_point_cloud("point_cloud.ply")

        # Convert the point cloud to a numpy array
        points = np.asarray(point_cloud_points)
        colors = np.asarray(point_cloud_colors)
        ...
        # o3d.io.write_point_cloud("selected_points.ply", selected_pcd)
    ...
    def sync_capture(self):
        ...
        for i in range(int(reps)):
            ...
            pcd_combined.colors = o3d.utility.Vector3dVector([*pcd_left_crop.colors, *pcd_right_crop.colors])
            
            
        
            colors = o3d.utility.Vector3dVector(img_left[:,:,:3].tolist())
            pcd_combined.colors = colors
            # The folder name need to be mandatory as if there isn't a folder name it will return an error
            
            # use color filter
            low_color = [140, 140, 140]  # Lower bound of the color range in RGB format--- grey
            high_color = [255, 255, 255]  # Upper bound of the color range in RGB format---- white

            color_filter_pcd = self.color_filter(pcd_combined.points, pcd_combined.colors, low_color, high_color)


            np.savez(f'./DATA/{folder_name}/data_{i}.npz', pcd=color_filter_pcd, transformation=reg_p2p.transformation, img_l=img_left)
            # pcd=pcd_combined.points
            # pcd=color_filter_pcd
