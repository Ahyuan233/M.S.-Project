...
for filename in os.listdir(folder_path):
    ...
    if os.path.isfile(file_path):
        ...
        pcd_left_crop_for_ICP = pt_l.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([-550, -520, 250]),  # min np.array([-650, -720, 250])
                                                                                np.array([550, 300, 920])))       # max np.array([550, 300, 920])
        
        pcd_right_crop_for_ICP = pt_r.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([-550, -520, 250]), # min np.array([-500, -520, 250])
                                                                                    np.array([550, 300, 1150])))      # max np.array([500, 300, 1250])
