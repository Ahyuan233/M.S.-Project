...
opt_transformation = np.array([[8.38222388e-01, 1.25279844e-01, -5.30743054e-01, 1.01218193e+02],
                [-1.18683764e-01, 9.91834374e-01, 4.66769502e-02, 1.19643240e+01],
                [5.32256886e-01, 2.38649189e-02, 8.46246461e-01, 2.45292564e+01],
                [0.00000000e+00, 0.00000000e+00, 0.00000000e+00, 1.00000000e+00]])

# new_opt = np.array([[ 7.64114573e-01, -2.31955666e-01,  6.01934788e-01, -4.79399509e+02],
#  [ 2.02249071e-01,  9.72211278e-01 , 1.17900566e-01 ,-7.44071715e+01],
#  [-6.12555494e-01 , 3.16512108e-02  ,7.89793623e-01  ,1.32654591e+02],
#  [ 0.00000000e+00  ,0.00000000e+00 , 0.00000000e+00  ,1.00000000e+00]])

new_opt = np.array([[ 7.67418688e-01, -1.44888327e-01,  6.24560591e-01 ,-5.00516681e+02],
        [ 1.40515509e-01,  9.88456438e-01 , 5.66503450e-02, -3.42805982e+01],
        [-6.25558910e-01,  4.42859154e-02 , 7.78918999e-01 , 1.40798441e+02],
        [ 0.00000000e+00,  0.00000000e+00  ,0.00000000e+00,  1.00000000e+00]])

# new_opt =np.array([[ 7.29888613e-01, -1.72909702e-01,  6.61335655e-01, -4.97898356e+02],
#  [ 1.47544158e-01,  9.84523804e-01 , 9.45706110e-02 ,-5.79368463e+01],
#  [-6.67452871e-01 , 2.85502002e-02,  7.44104529e-01,  1.51993567e+02],
#  [ 0.00000000e+00 , 0.00000000e+00 , 0.00000000e+00 , 1.00000000e+00]])

# opt_1 = np.array([[0.86712083, -0.02652873, 0.4973909, -9.61540124],
#        [0.01693107, 0.99957344, 0.02379644, 0.0891671],
#        [-0.49781003, -0.01221302, 0.8672001, -6.95710324],
#        [0, 0, 0, 1]])
...
# ---start processing---
folder_path = "/home/nuc/Desktop/kinect_camera/DATA/exp1" #-------------------------change this to raw data's folder name    #  Black_screen low_light calibration_cube
for filename in os.listdir(folder_path):
    ...
    if os.path.isfile(file_path):
        ...
        # trans_depth_l = data['trans_depth_l']
        # trans_depth_r = data['trans_depth_r']
        ...
        # p1 = pcd_left.points[5000]
        # p2 = pcd_right.points[5001]
        ...
        # scale_factor_left = 1 / 0.0033214809652244316 # dist_orig     0.0033214809652244316
        # scale_factor_right = 1 / 0.0033214809652244316
        ...
        pcd_left_crop_for_ICP = pt_l.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([-320, -400, 350]), # min  x-70 is the left side of the slope, 55 is the right side of the slope ,y:-72 top of the ball, 70 bottom of the slope, z:from -150 near to 500 far, -105 is the position without camera plane, 210 is the start of slope
                                                                                  np.array([400, 350, 1200])))  # max
        
        pcd_right_crop_for_ICP = pt_r.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([-260, -400, 380]), # min  x-70 is the left side of the slope, 55 is the right side of the slope ,y:-72 top of the ball, 70 bottom of the slope, z:from -150 near to 500 far, -105 is the position without camera plane, 210 is the start of slope
                                                                                    np.array([500, 300, 1200])))  # max





        # pcd_left_crop_for_ICP = pcd_left.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([-90, -72, -105]), # min  x-70 is the left side of the slope, 55 is the right side of the slope ,y:-72 top of the ball, 70 bottom of the slope, z:from -150 near to 500 far, -105 is the position without camera plane, 210 is the start of slope
        #                                                                           np.array([90, 110, 210])))  # max
        
        # pcd_right_crop_for_ICP = pcd_right.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([-90, -72, -105]), # min  x-70 is the left side of the slope, 55 is the right side of the slope ,y:-72 top of the ball, 70 bottom of the slope, z:from -150 near to 500 far, -105 is the position without camera plane, 210 is the start of slope
        #                                                                             np.array([80, 110, 210])))  # max
        
        # pcd_left_crop_for_visual = pcd_left.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([-70, -72, -105]), # min  x-70 is the left dide of the slope, 55 is the right side of the slope ,y:-72 top of the ball, 70 bottom of the slope, z:from -150 near to 500 far, -105 is the position without camera plane, 210 is the start of slope
        #                                                                   np.array([50, 65, 210])))  # max
        #                                                                     #   min_bound: numpy.ndarray[numpy.float64[3, 1]], max_bound: numpy.ndarray[numpy.float64[3, 1]], # need to change range if scale change
        # pcd_right_crop = pcd_right.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([-500, -200, 1]), np.array([80, 200, 500]))) # (np.array([0, -400, 1]), np.array([80, 200, 500])))
        ...
        #-----------------------------------------------Downsampling----------------------------------------
        voxel_size = 10   # 0.05 means 5cm for this dataset, lower voxel size corresponds to higher resolution   # 1 is original size
        source_down, source_fpfh = preprocess_point_cloud(pcd_right_crop_for_ICP, voxel_size)
        target_down, target_fpfh = preprocess_point_cloud(pcd_left_crop_for_ICP, voxel_size)
        ...
        #-----------------------------------------------general ICP
        current_transformation = np.identity(4)
        IPC_init = new_opt # current_transformation   # result_ransac.transformation
        icp_threshold = 1000  # 1 makes difference max_correspondence_distance
        reg_p2p = o3d.pipelines.registration.registration_icp(
            source_down, target_down, icp_threshold, IPC_init,
            o3d.pipelines.registration.TransformationEstimationPointToPoint(),
            o3d.pipelines.registration.ICPConvergenceCriteria(relative_fitness=1.000000e-09, relative_rmse=1.000000e-09, max_iteration=100000))
        print("trans before icp", IPC_init)
        print("trans after icp", reg_p2p.transformation)
        # draw_registration_result_original_color(source_down, target_down, new_opt)
        # draw_registration_result_original_color(source_down, target_down, reg_p2p.transformation)
        draw_registration_result(source_down, target_down, new_opt)
        ...
        # # save processed data
        # np.savez(f'./DATA/{folder_name}/{filename}', depth_l=depth_l, depth_r=depth_r)
