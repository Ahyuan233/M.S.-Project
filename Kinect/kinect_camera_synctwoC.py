...
class KINECT():
    ...
    def sync_capture(self):
        ...
        for i in range(int(reps)):
            ...
            # open3dVisualizer_left(pt_left, img_left) #Unannotating this code may cause collapse of the vscode 
            ...
            right_down, right_fpfh = self.preprocess_point_cloud(pcd_right, voxel_size)#fpfh-->feature

            # self.draw_registration_result(left_down, right_down, np.identity(4))
            ...
            pcd_left_crop = pcd_left.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([-200, -400, 50]),
															   np.array([0, 200, 320])))
            pcd_right_crop = pcd_right.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([0, -400, 50]),
															   np.array([80, 200, 500])))
#----------------------------------------------------------
...
#----------------------------------------------------------
            ...
            # np.savez(f'./DATA/{folder_name}/data_{i}.npz', pcd=pcd_combined.points, transformation=reg_p2p.transformation, img_l=img_left)#d = np.load(path)
