...
class KINECT():
    ...
    def sync_capture(self):
        ...
        for i in range(int(repetition)):
            power = randint(0,1) # Change the numbers for the range of power. Try to not excede 75 percent power as in long tests the soft figner might deform
            time.sleep(10)
            ...
            pcd_left_crop = pcd_left.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([-200, -10, 1]),
															   np.array([-39, 150, 320])))
            pcd_right_crop = pcd_right.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([10, -50, 1]),
															   np.array([80, 150, 500])))
