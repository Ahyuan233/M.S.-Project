...
inverse = np.array([[ 9.99714424e-01,  2.34313354e-02,  4.69505918e-03, -3.41256908e+01],
                    [-2.33684157e-02,  1.27 ,         -1.30356508e-02,  1.00409961e+01],
...
# ---start processing---
folder_path = "/home/nuc/Desktop/kinect_camera/DATA/calib" #-------------------------change this to raw data's folder name    #  Black_screen low_light calibration_cube
for filename in os.listdir(folder_path):
    ...
    if os.path.isfile(file_path):
        ...
        # vis = o3d.visualization.Visualizer()
        # vis.create_window()
        # vis.add_geometry(pt_r) #pcd_right
        # vis.run()
        # vis.destroy_window()
        ...
        # p1 = pt_l.points[50000]
        # p2 = pt_l.points[50001]
        ...
        # scale_factor_left = 1 / 4.123105625617661 # dist_orig     
        # scale_factor_right = 1 / 0.0033214809652244316
        # pt_l.scale(scale_factor_left, center=pt_l.get_center())
        # pt_r.scale(scale_factor_right, center=pt_r.get_center())
        # l_center=pt_l.get_center()
        # r_center=pt_r.get_center()
        # print("l",l_center)
        # print("r",r_center)
        ...
        # pcd_left_crop_for_ICP = pt_l.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([-100, -220, 500]),  # min np.array([-550, -720, 250])   ### y is hight, change x and z only, 
        #                                                                         np.array([10, 200, 720])))       # max np.array([550, 300, 920])   center:[ 25.23762512 -96.48274231 342.63977432]
        
        # pcd_right_crop_for_ICP = pt_r.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([-70, -520, 550]), # min np.array([-500, -520, 250])
        #                                                                         np.array([50, 400, 850])))      # max np.array([500, 300, 1250])   center:[  -9.85006714 -176.27865601  454.47079468]
        
        pcd_left_crop_for_visual = pt_l.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([-650, -520, 250]),  # min np.array([-650, -720, 250])
                                                                                np.array([550, 300, 920])))       # max np.array([550, 300, 920])
        
        pcd_right_crop_for_visual = pt_r.crop(o3d.geometry.AxisAlignedBoundingBox(np.array([-500, -520, 250]), # min np.array([-500, -520, 250])
                                                                                    np.array([500, 300, 1250])))      # max np.array([500, 300, 1250])
        
        
        
        draw_registration_result(pcd_right_crop_for_visual, pcd_left_crop_for_visual, inverse)
        #---------------------------------------display--------------------------------------------
        # vis = o3d.visualization.Visualizer()
        # vis.create_window()
        # vis.add_geometry(pcd_left_crop_for_visual)
        # vis.run()
        # vis.destroy_window()

        # vis = o3d.visualization.Visualizer()
        # vis.create_window()
        # vis.add_geometry(pcd_right_crop_for_visual)
        # vis.run()
        # vis.destroy_window()
        ...
        #-----------------------------------------------Downsampling----------------------------------------
        # voxel_size = 5   # 0.05 means 5cm for this dataset, lower voxel size corresponds to higher resolution   # 5
        # source_down, source_fpfh = preprocess_point_cloud(pcd_left, voxel_size)
        # target_down, target_fpfh = preprocess_point_cloud(pcd_right, voxel_size)
        ...
        # current_transformation = np.identity(4)
        # IPC_init = current_transformation # current_transformation   # result_ransac.transformation
        # icp_threshold = 10  # 1 makes difference max_correspondence_distance
        ...
        # print("trans before icp", current_transformation)
        # print("trans after icp", reg_p2p.transformation)
        # draw_registration_result_original_color(pcd_right_crop_for_ICP, pcd_left_crop_for_ICP, current_transformation)
        # draw_registration_result_original_color(pcd_right_crop_for_ICP, pcd_left_crop_for_ICP, reg_p2p.transformation)
